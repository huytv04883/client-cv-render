 ...
 * Supports any markdown format - no fixed structure required
 */
export async function parseMarkdown(markdown: string): Promise<ParsedResume> {
  ...
  // 4. Detect content type and extract data for each section
  sections = sections.map((section) => extractSectionData(section));

  // 5. Extract header data from frontmatter or first section
...
function extractHeaderData(
  ...
  if (firstH1) {
    return {
      ...
      // Try to extract more from content
